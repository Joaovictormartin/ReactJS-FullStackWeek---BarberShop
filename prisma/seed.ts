...
async function seedDatabase() {
  try {
    // Limpa os dados do banco antes de adicionar novos
    ...
    const barbershopPromises = creativeNames.map(async (name, i) => {
      ...
      // Criação de serviços para cada barbearia
    ...
    const createdBarbershops = await Promise.all(barbershopPromises);

    console.log("Seed executado com sucesso! Barbearias criadas:");
    console.table(createdBarbershops);
  } catch (error) {
    console.error("Erro ao executar o seed:", error);
  } finally {
    await prisma.$disconnect(); // Garante o fechamento da conexão
  }
}
