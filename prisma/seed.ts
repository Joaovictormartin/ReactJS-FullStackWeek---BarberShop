...
async function seedDatabase() {
  try {
    ...
    // Executando em paralelo as criações de barbearias
    const barbershopPromises = creativeNames.map(async (name, i) => {
      ...
      // Executa todas as promessas de serviços para cada barbearia
      await Promise.all(servicePromises);

      return barbershop;
    });

    await Promise.all(barbershopPromises);
  } catch (error) {
    console.error("Erro ao executar o seed =>", error);
  } finally {
    await prisma.$disconnect(); // Garantir o fechamento da conexão
  }
}
